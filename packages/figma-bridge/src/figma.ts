...
<body>
  ...
  <div class="section">
    <div class="section-title">ℹ️ What will be imported</div>
    <div class="token-list">
      <div class="token-item">• Color styles for all color scales</div>
      <div class="token-item">• Text styles for all typography tokens</div>
      <div class="token-item">• Styles will be created or updated</div>
    </div>
  </div>
...
async function pushToFigma(
  ...
  try {
    ...
    console.log('✅ Connected to Figma file:', fileResponse.data.name);

    // Note: Figma API currently doesn't support creating styles programmatically
    // This would require a plugin or manual import
    console.log('⚠️  Note: Figma API does not support programmatic style creation');
    console.log('💡 Use plugin generation mode instead: ds figma --mode plugin');
  } catch (error: any) {
...
export async function figma(options: FigmaOptions): Promise<void> {
  ...
  if (options.mode === 'plugin') {
    console.log('🔌 Generating Figma plugin...');
    ...
    console.log('✅ Figma plugin generated!');
    console.log(`\n📦 Plugin files:`);
    ...
    console.log('⬆️  Pushing tokens to Figma via API...');
